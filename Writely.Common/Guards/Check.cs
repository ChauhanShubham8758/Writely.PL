...
namespace Writely.Common.Guards
{
    public class Check
    {
        internal Check()
        ...
        public class Argument
        {
            ...
            public static void IsNotEmpty(string argument, string argumentName)
            {
                if (string.IsNullOrEmpty((argument ?? string.Empty).Trim()))
                    throw new ArgumentException("\"{0}\" cannot be blank.".FormatWith(new object[] { argumentName }),
                        argumentName);
            }
            ...
            public static void IsNotEmpty(Guid argument, string argumentName)
            {
                if (argument == Guid.Empty)
                    throw new ArgumentException("\"{0}\" cannot be empty guid.".FormatWith(new object[] { argumentName }),
                        argumentName);
            }
            ...
            public static void IsNotInvalidEmail(string argument, string argumentName)
            {
                ...
                if (!argument.IsEmail())
                    throw new ArgumentException(
                        "\"{0}\" is not a valid email address.".FormatWith(new object[] { argumentName }), argumentName);
            }
            ...
            public static void IsNotInvalidWebUrl(string argument, string argumentName)
            {
                ...
                if (!argument.IsWebUrl())
                    throw new ArgumentException(
                        "\"{0}\" is not a valid web url.".FormatWith(new object[] { argumentName }), argumentName);
            }
            ...
            public static void IsNotOutOfLength(string argument, int length, string argumentName)
            {
                if (argument?.Trim().Length > length)
                    throw new ArgumentException(
                        "\"{0}\" cannot be more than {1} character.".FormatWith(new object[] { argumentName, length }),
                        argumentName);
            }
            ...
            public static void IsNotOutOfRange(int argument, int min, int max, string argumentName)
            {
                if (argument < min || argument > max)
                    throw new ArgumentOutOfRangeException(argumentName,
                        "{0} must be between \"{1}\"-\"{2}\".".FormatWith(new object[] { argumentName, min, max }));
            }
        }
    }
}
